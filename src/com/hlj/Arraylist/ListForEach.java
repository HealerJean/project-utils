...
public class ListForEach {
    ...
    public static void main(String[] args) {
        ...
        while(iterator.hasNext()){       //刚进来cursor为0 
            String str = (String) iterator.next();           //第一次next之后游标为1，指向的是下一个索引，与list对象size判断是否有未迭代的对象
...
//              list.remove(it.next()); 如果用list.remove(it.next());会报异常checkForComodification
                it.remove();//remove的是b remove之后cursor会减1,因为cursor指向的是下一个索引，删除之后下一个索引就会减1，然后list个数会减1
            }
